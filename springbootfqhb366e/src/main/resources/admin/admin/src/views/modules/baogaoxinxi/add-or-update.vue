<template>
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'" label="报告编号" prop="baogaobianhao">
					<el-input v-model="ruleForm.baogaobianhao" placeholder="报告编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.baogaobianhao" label="报告编号" prop="baogaobianhao">
					<el-input v-model="ruleForm.baogaobianhao" placeholder="报告编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="报告名称" prop="baogaomingcheng">
					<el-input v-model="ruleForm.baogaomingcheng" placeholder="报告名称" clearable  :readonly="ro.baogaomingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="报告名称" prop="baogaomingcheng">
					<el-input v-model="ruleForm.baogaomingcheng" placeholder="报告名称" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="报告类型" prop="baogaoleixing">
					<el-select :disabled="ro.baogaoleixing" v-model="ruleForm.baogaoleixing" placeholder="请选择报告类型" >
						<el-option
							v-for="(item,index) in baogaoleixingOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="报告类型" prop="baogaoleixing">
					<el-input v-model="ruleForm.baogaoleixing"
						placeholder="报告类型" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.wenjianfujian" label="文件附件" prop="wenjianfujian">
					<file-upload
						tip="点击上传文件附件"
						action="file/upload"
						:limit="1"
						:type="3"
						:multiple="true"
						:fileUrls="ruleForm.wenjianfujian?ruleForm.wenjianfujian:''"
						@change="wenjianfujianUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.wenjianfujian" label="文件附件" prop="wenjianfujian">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"#18c1b9","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.wenjianfujian)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.wenjianfujian" label="文件附件" prop="wenjianfujian">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="负责人账号" prop="fuzerenzhanghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.fuzerenzhanghao" label="负责人账号" prop="fuzerenzhanghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="报告内容" prop="baogaoneirong">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.baogaoneirong" label="报告内容" prop="baogaoneirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.baogaoneirong}}</span>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='baogaoleixing'){
							this.ruleForm.baogaoleixing = obj[o];
							this.ro.baogaoleixing = true;
							continue;
						}
						if(o=='wenjianfujian'){
            ...
            this.$http({
				url: `option/baogaoleixing/baogaoleixing`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.baogaoleixingOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
            this.$http({
				url: `option/fuzeren/fuzerenzhanghao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
			...
			fuzerenzhanghaoChange () {
				this.$http({
					url: `follow/fuzeren/fuzerenzhanghao?columnValue=`+ this.ruleForm.fuzerenzhanghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
	...
	if(this.ruleForm.wenjianfujian!=null) {
		this.ruleForm.wenjianfujian = this.ruleForm.wenjianfujian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    wenjianfujianUploadChange(fileUrls) {
	    this.ruleForm.wenjianfujian = fileUrls;
    },
  }
};
</script>
